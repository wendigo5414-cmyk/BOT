...
spawn(function()
    ...
    checkSavedKey()
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: "Click the button below to copy loadstring!" })
            .setTimestamp();

        // Create copy button
        const copyButton = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                .setCustomId(`copy_loadstring_${Date.now()}`)
                .setLabel("📋 Copy Loadstring")
                .setStyle(ButtonStyle.Success)
        );

        const replyMsg = await message.reply({ embeds: [embed], components: [copyButton] });

        // Button collector to handle copy action
        const collector = replyMsg.createMessageComponentCollector({ time: 300000 }); // 5 minutes

        collector.on('collect', async (interaction) => {
            if (interaction.customId.startsWith('copy_loadstring')) {
                await interaction.reply({ 
                    content: `✅ **Loadstring Copied!**\n\`\`\`lua\n${loadstringCode}\n\`\`\`\nPaste this in your executor!`, 
                    ephemeral: true 
                });
            }
        });
        ...
        collector.on('end', () => {
            ...
            replyMsg.edit({ embeds: [embed], components: [copyButton] }).catch(() => {});
        });
