...
client.on("interactionCreate", async (interaction) => {
    ...
    // Handle ?lsd delete all confirmation
    ...
    // Handle ?lsd single script deletion
...
spawn(function()
    ...
    if (cmd === "?lsd" || cmd === "?lsdelete" || cmd === "?lsdel" || cmd === "?loadingstringdelete") {
        ...
        if (input.toLowerCase() === "all") {
            ...
            return message.reply({
                content: "⚠️ **WARNING:** This will delete ALL scripts from the repository!",
                components: [row]
            });
        }
    ...
    if (cmd === "?setlogs") {
        // Owner only command
        if (message.author.id !== OWNER_ID) {
            return message.reply("❌ Only the bot owner can set log channel.");
        }
